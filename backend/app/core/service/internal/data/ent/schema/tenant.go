...
func (Tenant) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Enum("status").Comment("租户状态").NamedValues("On", "ON", "Off", "OFF", "Expired", "EXPIRED", "Freeze", "FREEZE").Default("ON").Optional().Nillable(),
		field.Enum("type").Comment("租户类型").NamedValues("Trial", "TRIAL", "Paid", "PAID", "Internal", "INTERNAL", "Partner", "PARTNER", "Custom", "CUSTOM").Default("PAID").Optional().Nillable(),
		field.Enum("audit_status").NamedValues("Pending", "PENDING", "Approved", "APPROVED", "Rejected", "REJECTED").Default("PENDING").Optional().Nillable(),
