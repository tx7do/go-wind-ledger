...
func NewCategoryServiceClient(ctx *bootstrap.Context, r registry.Discovery) ledgerV1.CategoryServiceClient {
	cli, err := rpc.CreateGrpcClient(ctx.Context(), r, serviceid.NewDiscoveryName(serviceid.CoreService), ctx.GetConfig())
	if err != nil {
		return nil
	}
...
func NewTagServiceClient(ctx *bootstrap.Context, r registry.Discovery) ledgerV1.TagServiceClient {
	cli, err := rpc.CreateGrpcClient(ctx.Context(), r, serviceid.NewDiscoveryName(serviceid.CoreService), ctx.GetConfig())
	if err != nil {
		return nil
	}
