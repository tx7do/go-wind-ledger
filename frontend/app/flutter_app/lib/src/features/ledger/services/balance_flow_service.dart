...
class BalanceFlowService extends BaseService {
  ...
  Future<dynamic> get(int id) async {
    try {
      return await _api.get(LedgerServiceV1GetBalanceFlowRequest(id: id));
    } on DioException catch (e) {
      return handleDioError(e);
    }
  }
  ...
  /// 创建流水
  Future<dynamic> create(BalanceFlow data) async {
    try {
      return await _api
  ...
  Future<dynamic> delete(int id) async {
    try {
      await _api.delete(LedgerServiceV1DeleteBalanceFlowRequest(id: id));
      return null;
    } on DioException catch (e) {
      return handleDioError(e);
    }
  ...
  /// 确认流水（更新账户余额）
  Future<dynamic> confirm(int id) async {
    try {
      return await _api
